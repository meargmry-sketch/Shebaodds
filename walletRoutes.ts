...
// ⚠️ INTEGRATION BOUNDARY — READ BEFORE MODIFYING ⚠️
// processDeposit/processWithdrawal below are SIMULATED gateway calls. They always
// return success and never touch a real payment rail. This is intentional.
//
// A real deployment needs one of:
//   (a) a licensed payment service provider's API/SDK (card, bank, or mobile-money
//       gateway) plugged in here, with webhook-verified callbacks — not
//       admin-typed confirmation — as the source of truth for "did this payment
//       actually happen", or
//   (b) a manual mobile-money reconciliation workflow (e.g. Telebirr/CBE Birr)
//       where the admin ticket-review screen (see AdminDepositReview.jsx) is
//       wired to the existing /api/admin/transactions/:id/approve endpoint in
//       adminRoutes.ts — and even then, the ticket number should be checked
//       against a verified statement/webhook, not trusted purely because an
//       admin typed a matching string.
// Either path requires the operator to hold the relevant payment/gambling
// licensing for their jurisdiction before going live.
